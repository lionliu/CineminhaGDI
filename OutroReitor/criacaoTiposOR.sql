...
ALTER TYPE TP_ENDERECO
 ...
 CREATE OR REPLACE TYPE TP_PESSOA AS OBJECT(
    ...
    L_FONES TP_FONES,
    MEMBER FUNCTION comparaCPF (p TP_PESSOA) RETURN INTEGER
)NOT FINAL;
/
...
CREATE OR REPLACE TYPE TP_FUNCIONARIO UNDER TP_PESSOA(
  ID_FUNCIONARIO NUMBER /*NOT NULL*/,
    ...
    SALARIO DECIMAL(6,2),
    OVERRIDING MEMBER FUNCTION comparaCPF (p TP_PESSOA) RETURN INTEGER
...
CREATE OR REPLACE TYPE TP_SALA AS OBJECT(
    ID_SALA NUMBER,
    CAPACIDADE NUMBER,  --,
    CONSTRUCTOR FUNCTION TP_SALA (ID_SALA NUMBER, CAPACIDADE NUMBER) RETURN SELF AS RESULT,
...
CREATE OR REPLACE TYPE TP_ATOR2 AS OBJECT( 
    NOME_ATOR VARCHAR(30)    
...
OVERRIDING MEMBER FUNCTION comparaCPF (p TP_PESSOA)  RETURN INTEGER IS
   m TP_FUNCIONARIO;
      r integer;
     begin
        if p is of ( TP_FUNCIONARIO ) then
           m := treat( p as TP_FUNCIONARIO );
           r :=
             case
                when self.CPF = m.CPF then 0
                 when self.CPF > m.CPF then 1
                when self.CPF < m.CPF then -1
               end;
       end if;
   return r;
...
CREATE OR REPLACE TYPE BODY TP_SALA AS
 CONSTRUCTOR FUNCTION TP_SALA 
   (ID_SALA NUMBER, CAPACIDADE NUMBER) RETURN SELF AS RESULT IS
  BEGIN
   ID_SALA := i; CAPACIDADE := c; 
  END;
END;
/                                     

CREATE OR REPLACE TYPE BODY TP_SALA AS --melhorar aqui
